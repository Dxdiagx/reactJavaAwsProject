...
class FileGridList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      index:'', isModalOpen: false, shareEmail:'', file:'' , group:'', downloadLink:'',
      email:''
    };
  }
  componentWillMount() {
  ...
  const generateDownloadLink = (bucketName, fileName) => {
    const params = {
      Bucket: bucketName,
      Key: fileName,
      Expires: 60, // İndirme bağlantısının geçerlilik süresi (saniye cinsinden)
    };
  
    return s3.getSignedUrl('getObject', params);
  };
  var downloadlink = generateDownloadLink('bitirmes3',this.state.email+'/'+file.filename);
